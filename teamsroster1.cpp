...
//	SetSizerAndFit( main );
	...
	Bind( EVT_ASSIGN_DRAFTED_PLAYERS, &CTeamsRoster::OnAssignDraftedPlayers, this );
}
...
CTeamsRoster::~CTeamsRoster(void)
{
	...
	Unbind( EVT_ASSIGN_DRAFTED_PLAYERS, &CTeamsRoster::OnAssignDraftedPlayers, this );
...
void CTeamsRoster::PerformDraft(const CPlayer &player)
{
	...
	m_roster->BeginBatch();
/*	for( i = 0; i < m_roster->GetNumberCols() && column == -1; i++ )
	{
		if( m_roster->GetColLabelValue( i ) == const_cast<CPlayer &>( player ).GetOwner() )
			column = i;
	}
	for( i = 0; i < m_roster->GetNumberRows() && row == -1; i++ )
	{
		if( m_roster->GetRowLabelValue( i ) == const_cast<CPlayer &>( player ).GetDraftedPosition() && m_roster->GetCellValue( i, column ).IsEmpty() )
			row = i;
	}
	m_roster->SetCellValue( row, column, wxString::Format( "%d %s", player.GetAmountPaid(), const_cast<CPlayer &>( player ).GetName() ) );
	m_playersLeft[column]--;
	m_salary[column] -= player.GetAmountPaid();
	m_maxBid[column] = m_salary[column] - ( m_playersLeft[column] - 1 );
	m_average[column] = m_salary[column] / m_playersLeft[column];
...
void CTeamsRoster::OnAssignDraftedPlayers(wxCommandEvent &event)
{
	int col, price;
	bool found = false;
